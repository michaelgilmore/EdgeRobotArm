...
/**
 * Communicates with the OWI-535 Robot Arm via USB: libusb-win32
 *
 * @author Andrew Davison
 *
 * modified by Craig Turner 19/01/2013 
 * @see <http://gampageek.blogspot.co.uk/2013/01/blog-post.html>
 * 
 * This Java coded utilises the libraries
 * libusbjava
 *
 * 
 * @see <http://sourceforge.net/apps/trac/libusb-win32/wiki
 *
 */

/* ArmCommunicator.java Andrew Davison <ad@fivedots.coe.psu.ac.th> June 2011 *
 * 
 *  
 */
...
/* The robot arm ops are spread over three bytes:
 /
 * First byte: Gripper close == 0x01 Gripper open == 0x02 Wrist forwards == 0x04
 * Wrist backwards == 0x08 Elbow forwards == 0x10 Elbow backwards == 0x20
 * Shoulder forwards == 0x40 Shoulder backwards == 0x80
 *
 * Second byte: Base rotate right == 0x01 Base rotate left == 0x02
 *
 * Third byte: Light on == 0x01
 *
...
public class ArmCommunicator
{
    ...
    private final static int OPEN_GRIPPER_PERIOD = 50;   // ms time to open
    private final static int CLOSE_GRIPPER_PERIOD = 50;   // ms time to close
    ...
    public void openGripper(boolean isOpen)
    {
        if (isOpen)
        {
            ...
            sendCommand(0x02, 0x00, OPEN_GRIPPER_PERIOD);
        } else
        {
            System.out.println("  Gripper: close");
            sendCommand(0x01, 0x00, CLOSE_GRIPPER_PERIOD);
        }
    }  // end of openGripper()
     ...
     * @param turnOn
     */
    public void setLight(boolean turnOn) // Third byte: Light on/off
    {
        isLightOn = turnOn;
    ...
    private int getLightVal(boolean isLightOn) // light on/off
    {
     ...
     * @param period
     */
    public void turn(JointID jid, int dir, int period) /* First byte:
    ...
    private void sendCommand(int opCode1, int opCode2, int period) // execute the operation for period millisecs
    {
    ...
    private void sendControl(int opCode1, int opCode2, int opCode3) // send a USB control transfer
    {
        /*
    ...
    private String toHexString(int b) // chanage the hexadecimal integer into "0x.." string format
    {
     ...
     * @param ms
     */
    public void wait(int ms) // sleep for the specified no. of millisecs
    {
    ...
    public static void doArmOp(char ch, ArmCommunicator arm) // use POSITIVE for forwards/right turns; NEGATIVE for backwards/left turns
    {
    ...
    public static void ArmTests(char ch, ArmCommunicator arm) // use POSITIVE for forwards/right turns; NEGATIVE for backwards/left turns
    {

        if (ch == '1') // gripper close
        {
            arm.openGripper(false);
            arm.openGripper(true);
        }
